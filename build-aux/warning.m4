...
# GCN_COMPILER_FLAGS_NAME
# ----------------------
# Return the name of the FLAGS variable for the current language.
m4_define([GCN_COMPILER_FLAGS_NAME],
[AC_LANG_CASE([C],   [CFLAGS],
              [C++], [CXXFLAGS])[]dnl
])# GCN_COMPILER_FLAGS_NAME


# GCN_COMPILER_OPTION_IF(OPTION, [IF-SUPPORTED], [IF-NOT-SUPPORTED])
# -----------------------------------------------------------------
...
AC_DEFUN([GCN_COMPILER_OPTION_IF],
[AS_VAR_PUSHDEF([ac_Option],
                [ac_cv_[]_AC_LANG_ABBREV[]_supports_$1])dnl
AC_CACHE_CHECK([whether _AC_LANG compiler accepts $1],
               [ac_Option],
[ac_save_[]GCN_COMPILER_FLAGS_NAME=$GCN_COMPILER_FLAGS_NAME
GCN_COMPILER_FLAGS_NAME="$GCN_COMPILER_FLAGS_NAME $1"
ac_Option=no
...
GCN_COMPILER_FLAGS_NAME=$ac_save_[]GCN_COMPILER_FLAGS_NAME
])
AS_IF([test AS_VAR_GET(ac_Option) = yes], [$2], [$3])[]dnl
AS_VAR_PUSHDEF([ac_Option])dnl
])# GCN_COMPILER_OPTION_IF


# GCN_CXX_WARNINGS(OPTIONS)
# ------------------------
...
# order.
AC_DEFUN([GCN_CXX_WARNINGS],
[ac_save_compiler_flags=$GCN_COMPILER_FLAGS_NAME
m4_foreach([AC_Option], [$1],
    [GCN_COMPILER_OPTION_IF(AC_Option,
           [WARNING_CXXFLAGS="$WARNING_CXXFLAGS AC_Option"
            GCN_COMPILER_FLAGS_NAME="$ac_save_compiler_flags $WARNING_FLAGS"])])
GCN_COMPILER_FLAGS_NAME=$ac_save_compiler_flags
